...
2. 已在「已知清單」中的實體，只回名字，known 設 true，不要重寫說明。
3. 新實體 known 設 false，並提供：
   - name_en：英文名（如 David、Bethlehem）
   - desc：60–100 字繁體中文說明，工具書語氣、客觀，含身份簡述＋在聖經中的角色。
   - 若是 place，再給 lat、lng（該地點的近似經緯度，浮點數；不確定就省略）。
4. 不要抽太泛的稱謂：以色列、耶和華、神、主 一律略過。
5. 嚴格只輸出 JSON，格式：
{"entities":[{"name":"大衛","type":"person","known":false,"name_en":"David","desc":"...","lat":null,"lng":null}]}
不要任何 JSON 以外的文字。"""
            ...
            try:
                ...
                for ent in result.get("entities", []):
                    ...
                    if name in entities:
                        continue  # 已有，保留首次說明
                    if ent.get("known"):
                        continue  # 模型誤標 known 但我們沒有 → 跳過，下次出現再補
                    etype = ent.get("type")
